...
- 🔍 **автоподбор стратегии** — перебирает варианты и сам выбирает рабочий по связи;
- 📡 **проверка связи** прямо в панели — видно, работает обход или нет (реальный TCP-пинг);
- 🔔 **иконка в трее** — свернуть в фон, вкл/выкл и переключить LMU из меню;
- 🚀 **автозапуск с Windows** — стартует при входе, без надоедливого UAC (через Планировщик задач);
...
2. **Двойной клик по нужному файлу** (права администратора попросит сам):
   - `general.bat` — обычный обход (Discord, YouTube, общий список);
   - `Le Mans Ultimate.bat` — то же **плюс** профиль LMU для онлайна;
   - `Всё сразу.bat` — **максимум**: обход + LMU + общий игровой фильтр для любых игр.
...
Не хочешь перебирать вручную — нажми в панели **«🔍 Автоподбор»**: он прогонит все варианты, проверит связь на каждом и оставит лучший (больше доступных сервисов, меньше пинг). Занимает ~10-15 секунд, связь в это время может моргать.
